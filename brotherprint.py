...
class BrotherPrint:
    ...
    def select_font(self, font):
        ...
        if font in fonts:
            if font in ['brougham', 'lettergothicbold', 'brusselsbit', 'helsinkibit', 'sandiego']:
                self.fonttype = self.font_types['bitmap']
            else:
            ...
            raise RuntimeError('Invalid font in function selectFont' + str(fonts))
    ...
    def barcode(self, data, format, characters='off', height=48, width='small', parentheses='on', ratio='3:1', equalize='off', rss_symbol='rss14std', horiz_char_rss=2):
        ...
        if(not isinstance(data, str)): # Python 3
            data = data.decode('windows-1252', 'replace') # TODO: Correct encoding on Python 3.
